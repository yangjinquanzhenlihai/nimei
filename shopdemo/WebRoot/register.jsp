...
<script type="text/javascript" src="jquery-3.2.1.min.js"></script>
	...
	var a=false,b=false,c=false,d=false,e=false,f=false;
		$(document).ready(function(){
	       ...
	       // var namemsg=document.getElementById("namemsg");
			if (uname.length<2||uname.length>20) {
				...
				$("#namemsg").html("用户名格式不对");
				// namemsg.setAttribute("class","input_ok");
				return a;
			}
			else{
				$.ajax({
					...
					data:"opr=checkName&uname="+$("#uname").val(),
					success:function(result){
						...
						if (result==="OK") {
							...
							return a;
							//namemsg.setAttribute("class","input_error");
						}
					}
				});			
			}	
		...
		if (password.length<6||uname.length>33){
		    
			$("#passwordtip").html("密码长度不对");
		   return b;
		}
		else{
			$("#passwordtip").html("OK");
		...
		$("#repasswordtip").css({"color":"red"});
		if (password.length==0) {
			
			$("#repasswordtip").html("请先设置密码");
		  ...
		  $("#phoneNum").blur(function(){
		  	...
		  	$("#phoneNumtip").css({"color":"red"});
		  	if (phoneNum.length!=11) {
				
			    $("#phoneNumtip").html("手机号不对，应有11位");
	...
	function change(){  //刷新验证码
	var captchaImg=document.getElementById("captchaImg");
	captchaImg.src="<c:url value='VeriServlet'/>?"+new Date().getTime();
	}
				...
				<div class="box clearfix">
					<form action="<c:url value='/UserServlet'/>" method="post" id="form1">
						...
						<table class="form_table f_l">
							...
							<tr>
								<th>用户名：</th>
								<td><input  class="gray" name='account' id="uname"
									type="text" value="" tabindex="1" /><label id="namemsg" class="">请填写用户名，格式为2-20个字符，可以为字数，数字下划线和中文</label></td>
							</tr>
							<tr>
								<th>设置密码：</th>
								<td><input class="gray" type="password" id="password"
									name='password' tabindex="2" /><label id="passwordtip">填写登录密码，6-32个字符</label></td>
							</tr>
							<tr>
								<th>确认密码：</th>
								<td><input class="gray" type="password" id="repassword" name='repassword'
									tabindex="3" /><label id="repasswordtip">重复上面所填写的密码</label></td>
							</tr>
							<tr>
								<th>手机号码：</th>
								<td><input class="gray" id="phoneNum" name='phoneNum' type="text" tabindex="4" ><label id="phoneNumtip"></label></td>
							</tr>
							<tr>
								<th>电子邮箱：</th>
								<td><input class="gray" id="email" name='email' type="text" tabindex="5" ><label id="emailtip"></label></td>
							</tr>
							<tr><th>验证码：</th><td><input type='text' id="captcha" class='gray_s' name='captcha' alt='填写下面图片所示的字符' />${msg}
							<label id="captchatip">填写下面图片所示的字符</label></td>
							</tr>
							<tr class="low"><th></th><td><img src="<c:url value='/VeriServlet'/>" id='captchaImg' />
							<span class="light_gray">看不清？<a class="link" href="javascript:change();">换一张</a></span>
							</td>
							</tr>
							<tr>
								<td></td>
								<td><input id="Submit" class="submit_reg" type="button"
