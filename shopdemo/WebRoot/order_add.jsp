...
<body class="second">
	<div class="brand_list container_2">
		...
		<div class="wrapper clearfix">
			...
			<form action='OrderFormServlet?opr=orderAdd' method='post' name='order_form'>
								...
								<ul class="addr_list">
									<c:forEach items="${addresses}" var="address">
										<li><label><input class="radio"
												...
												<c:if test="${address.selected eq '1'}">checked</c:if> />${address.name}&nbsp;&nbsp;&nbsp;&nbsp;
								...
								<tbody id="deliveryFormTrBox">
									<tr>
										<th><label><input type="radio"
												name="order.deliveryType" paytype="0" alt="20.00" value="快递"
												checked>快递</label></th>
							...
							<table width="100%" class="cart_table t_c">
								...
								<tbody>
									...
									<c:forEach items="${orderDetails}" var="orderDetail"
										...
										<tr>
											<td><img src="${pageContext.request.contextPath }/${orderDetail.goods.gAvatar}"
												width="66px" height="66px" alt="${orderDetail.goods.name}"
												title="${orderDetail.goods.name}" /></td>
											<td class="t_l" style="text-align:center"><a href="" class="blue">${orderDetail.goods.name}</a></td>
											<td>￥<b>${orderDetail.goods.spprice}</b></td>
											<td>${orderDetail.nums}</td>
											<td>￥<b class="red2">${orderDetail.goods.spprice*orderDetail.nums}</b></td>
											<c:set var="totalMoney"
												value="${totalMoney+orderDetail.goods.spprice*orderDetail.nums }"></c:set>
										</tr>
									</c:forEach>
									<!-- 商品展示 结束-->
